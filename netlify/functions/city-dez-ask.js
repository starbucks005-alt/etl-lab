/* city-dez-ask - sync trigger for Desmond "Dez" Quigley (city services). Member auth required. */
...
function newJobId() {
  ...
  return 'dq-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  const bgUrl = base + '/.netlify/functions/city-dez-background';
  ...
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ok: true, job_id: jobId, polling_endpoint: '/.netlify/functions/city-dez-status?job_id=' + jobId }),
  };
};
