/* city-tessa-ask - public, no auth. Sync trigger for Tessa Whitfield (city services). */

function newJobId() {
  const d = new Date();
  const stamp = d.toISOString().replace(/[-:.TZ]/g, '').slice(0, 14);
  return 'tw-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  const bgUrl = base + '/.netlify/functions/city-tessa-background';
  ...
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ok: true, job_id: jobId, polling_endpoint: '/.netlify/functions/city-tessa-status?job_id=' + jobId }),
  };
};
