/* csuite-alistair-ask — sync trigger for Alistair Montgomery's CSO backpack. */
...
function newJobId() {
  ...
  return 'am-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  const bgUrl = base + '/.netlify/functions/csuite-alistair-background';
  ...
  return {
    ...
    body: JSON.stringify({ ok: true, job_id: jobId, polling_endpoint: '/.netlify/functions/csuite-alistair-status?job_id=' + jobId }),
  };
};
