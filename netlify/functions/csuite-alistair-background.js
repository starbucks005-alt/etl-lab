/* ─────────────────────────────────────────────────────────────────────────────
   csuite-alistair-background

   Alistair Montgomery's CSO backpack. Owns market direction + competitive
   intelligence. v1: web_search; v2 adds FRED + Google Trends + SEC EDGAR
   + web_fetch per CCW's spec.
  ...
  for (const p of candidates) {
    try {
      if (fs.existsSync(p)) {
        ...
        return list.find(x => (x.id === 'alistair_montgomery' || x.name === 'Alistair Montgomery')) || null;
      }
    } catch (_) {}
  }
  return null;
...
exports.handler = async function(event) {
  ...
  const jobKey = 'alistair/' + jobId;
  await store.setJSON(jobKey, {
    job_id: jobId,
    agent: 'Alistair Montgomery',
    role: 'CSO',
    question,
    status: 'running',
  ...
  const systemPrompt = backpack.system_prompt + '\n\nWhen you answer:\n- Lead with the strategic call the owner should make and why.\n- Then the supporting market evidence with sources and dates.\n- Distinguish trend (durable shift) from fad (12-18 month wave).\n- Plain English. When using strategy jargon (TAM/SAM/SOM, moat, switching cost, network effect), translate on first use.\n- Avoid em dashes. Use commas, periods, semicolons.\n- Name the competitors by name when relevant. Date their moves precisely.\n- End with one line: "Bottom line:" followed by your one-sentence call.';
  ...
  try {
    ...
    console.error('[csuite-alistair-background] error', err && err.message);
    await store.setJSON(jobKey, {
      job_id: jobId,
      status: 'error',
