/* ─────────────────────────────────────────────────────────────────────────────
   csuite-benjamin-ask — sync trigger for Benjamin Reed's CISO backpack.
   Mints job_id, fires csuite-benjamin-background server-to-server, returns
   immediately. UI polls via csuite-benjamin-status.
   ───────────────────────────────────────────────────────────────────────────── */
...
function newJobId() {
  ...
  return 'br-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  const bgUrl = base + '/.netlify/functions/csuite-benjamin-background';
  ...
  return {
    ...
    body: JSON.stringify({ ok: true, job_id: jobId, polling_endpoint: '/.netlify/functions/csuite-benjamin-status?job_id=' + jobId }),
  };
};
