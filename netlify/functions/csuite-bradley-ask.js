/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Sync trigger for Bradley's backpack. UI POSTs a question, this function
   mints a job_id, fires csuite-bradley-background, returns the job_id
   immediately so the UI can start polling status.

   POST body: { question, context? }
...
function newJobId() {
  ...
  return 'bcs-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  // Fire background server-to-server, pass through the user's JWT so the
  // background's own auth gate validates.
  ...
  const bgUrl = base + '/.netlify/functions/csuite-bradley-background';

  // Fire-and-forget; do not await the response (background can run 15 min)
  try {
    fetch(bgUrl, {
      method: 'POST',
  ...
  return {
    ...
    body: JSON.stringify({ ok: true, job_id: jobId, polling_endpoint: '/.netlify/functions/csuite-bradley-status?job_id=' + jobId }),
  };
};
