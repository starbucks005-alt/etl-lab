/* ─────────────────────────────────────────────────────────────────────────────
   csuite-bradley-status

   UI polls this every 2-3 seconds while waiting for Bradley's answer.
   Returns { status: 'running'|'done'|'error', response?, error? }.
...
exports.handler = async function(event) {
  ...
  const job = await store.get('bradley/' + jobId, { type: 'json' });
  if (!job) {
    return {
      statusCode: 200,
  ...
  // Owner check: a job is only readable by the user who created it.
