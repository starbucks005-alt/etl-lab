/* ─────────────────────────────────────────────────────────────────────────────
   csuite-fiona-background

   Fiona Gallagher's GC / CCO backpack. Owns regulatory posture and risk
   exposure. v1: web_search only; v2 adds Regulations.gov + CourtListener
   + GovInfo per CCW's spec.

   IMPORTANT: This is NOT legal advice. Fiona research-grade, not counsel.
   System prompt + UI both surface this clearly.
  ...
  for (const p of candidates) {
    try {
      if (fs.existsSync(p)) {
        ...
        return list.find(x => (x.id === 'fiona_gallagher' || x.name === 'Fiona Gallagher')) || null;
      }
    } catch (_) {}
  }
  return null;
...
exports.handler = async function(event) {
  ...
  const systemPrompt = backpack.system_prompt + '\n\nCRITICAL DISCLAIMERS YOU MUST APPLY:\n- You are NOT a substitute for a licensed attorney. This is regulatory research, not legal advice. Surface this clearly at the start of any answer that touches a live legal dispute, contract drafting, or anything time-sensitive.\n- For any matter that could result in litigation, regulator action, or personal liability, recommend the owner consult a licensed attorney in their jurisdiction.\n- Use jurisdiction-specific language. Federal vs state vs local matters; do not generalize.\n\nWhen you answer:\n- Lead with the decision the owner should make and why.\n- Then supporting facts with sources and dates.\n- Plain English. Translate legal acronyms (HIPAA, GDPR, CCPA, COPPA, FERPA, CAN-SPAM, GLBA, SOX, etc.) on first use.\n- When citing a statute, give the section. When citing case law, give parties, court, and year.\n- Avoid em dashes. Use commas, periods, semicolons.\n- For regulatory windows (comment periods, effective dates, sunset clauses), give the specific date.\n- End with one line: "Bottom line:" followed by your one-sentence call.';
  ...
  try {
    ...
    console.error('[csuite-fiona-background] error', err && err.message);
    await store.setJSON(jobKey, {
      job_id: jobId,
      status: 'error',
