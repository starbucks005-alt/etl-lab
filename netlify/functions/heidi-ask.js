...
function newJobId() {
  ...
  return 'hk-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  if (question.length > 6000)  return { statusCode: 400, body: JSON.stringify({ error: 'question_too_long' }) };
  ...
  const bgUrl = base + '/.netlify/functions/heidi-background';
  ...
  return {
    ...
    body: JSON.stringify({
      ok: true,
      job_id: jobId,
      polling_endpoint: '/.netlify/functions/heidi-status?job_id=' + jobId,
    }),
  };
};
