/* ─────────────────────────────────────────────────────────────────────────────
   kronborg-room-background -- the actual multi-agent cascade for the Kronborg
   Table, moved out of the synchronous kronborg-room.js dispatcher. This is a
   Netlify Background Function (the "-background" filename suffix is what
   grants the extended, ~15-minute execution budget; no netlify.toml entry
   needed), because a 3-agent cascade (director pick -> visitor-memory fetch
   -> agent turn, possibly with a tool call -> visitor-memory save, per beat,
   up to CASCADE_CAP beats) measured 15-20s per reply on the live site, past
   even the platform's real ceiling for synchronous functions regardless of
   what a netlify.toml timeout says. That was the "table lost connection"
   error visitors were hitting.

   Same dispatcher + background-worker + poll pattern already used elsewhere
   on the ETL campus (heidi-ask/heidi-background/heidi-status,
   agent-ask/agent-background/agent-status, etc.), job state kept in Netlify
   Blobs, not Supabase, matching that established local convention.

   POST { job_id, active_agents, transcript, message, visitor_name?,
          visitor_id?, agent_state? }
   Fired by kronborg-room.js via fire-and-forget fetch. Writes the result to
   the `kronborg_room_jobs` Blobs store at key `<job_id>`; kronborg-room-status
   reads it back for the frontend to poll.
   ───────────────────────────────────────────────────────────────────────────── */
...
async function pickNextSpeaker(client, activeAgents, transcript, beatIndex, forced) {
  ...
  try {
    ...
    console.error('[kronborg-room-background] director failed (non-fatal):', err.message);
    return forced ? activeAgents[0] : null;
  }
}
...
async function runCascade(activeAgents, transcript, visitorName, visitorId, serviceKey, rawAgentState) {
  ...
  for (let beat = 0; beat < CASCADE_CAP; beat++) {
    ...
    try {
      ...
      console.error('[kronborg-room-background] turn error for', speaker, err.message);
      continue;
    }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.error('[kronborg-room-background] fatal error:', err && err.message);
