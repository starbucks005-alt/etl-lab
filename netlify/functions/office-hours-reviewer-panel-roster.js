/* ─────────────────────────────────────────────────────────────────────────────
   office-hours-journal-reviewer-panel

   Faculty uploads their paper PDF (or pastes the text), names the paper type,
   SME area, and optionally a target journal. Returns a simulated journal
   reviewer panel: 2 to 3 reviewers (drawn from the Prep Room defense roster,
   recast as journal peer reviewers), each writing their own report in their
   own voice with their own disciplinary lens.

   Reviewer roster is the same as the thesis-defense panel in /prep-room,
   but here they're playing peer-reviewers, not committee members. The
   personas, voices, and disciplinary lenses carry over intact.

   POST /.netlify/functions/office-hours-journal-reviewer-panel
   Body: {
     paper_pdf: { type: 'application/pdf', data: '<base64>' } | null,
     paper_text: '<fallback if no PDF>' | null,
     paper_type: 'primary' | 'review' | 'systematic-review' | 'brief-report' | 'case-report' | 'perspective' | 'methods',
     sme: '<short subject-matter description>',
     journal: '<optional target journal>',
     reviewers: ['vv','az','cole'] | null  // optional explicit pick; otherwise auto-selected
   }
   ───────────────────────────────────────────────────────────────────────────── */
...
const MAX_PDF_BYTES = 6_000_000; // ~6MB base64 cap
...
/* ─── Reviewer roster (lifted from /prep-room, recast as journal peer reviewers) ───
   Each has a stable id, display name, role (lens), portrait paths, and the
   persona prompt that drives their voice. The defense-panel prompt is reused
   verbatim because the personas are the same humans; only the situation
   differs. We append a JOURNAL-REVIEW INSTRUCTION block to the system prompt
   per reviewer at call time.
   ─────────────────────────────────────────────────────────────────────────── */
const REVIEWERS = {
  vv: {
    name: 'Dr. Victoria Vance',
    ...
    best_for: ['primary', 'brief-report', 'methods'],
  },
  lc: {
    name: 'Dr. Lawrence Cole',
    ...
    best_for: ['primary', 'review', 'systematic-review', 'perspective'],
  },
  mt: {
    ...
    best_for: ['review', 'perspective', 'systematic-review'],
  },
  mp: {
    name: 'Dr. Maya Patel',
    role: 'The Skeptic',
    ...
    best_for: ['primary', 'perspective', 'methods', 'brief-report'],
  },
  cs: {
    name: 'Dr. Charles Sterling',
    role: 'The Supportive Editor',
    ...
    best_for: ['review', 'perspective', 'case-report', 'methods'],
  },
  az: {
    name: 'Dr. Alan Zhao',
    role: 'The Stats Hardliner',
    ...
    best_for: ['primary', 'systematic-review', 'methods', 'brief-report'],
  },
  mvg: {
    name: 'Dr. Meredith Vance-Giles',
    role: 'The Outside Reader',
    ...
    best_for: ['review', 'perspective', 'case-report', 'primary'],
  },
  tk: {
    name: 'Dr. Tariq Khan',
    role: 'The Interdisciplinary Bridge',
    ...
    best_for: ['review', 'perspective', 'primary', 'methods'],
  },
};
...
/* ─── Auto-selection ─────────────────────────────────────────────────────────
   Pick 3 reviewers given paper_type. We always include one Methodologist or
   Stats Hardliner, one Domain or Theorist (depending on paper type), and one
   Outside Reader or Skeptic for the audience check. Editor and bridge
   reviewers are bench picks.
   ─────────────────────────────────────────────────────────────────────────── */
...
function buildReviewerSystemPrompt(reviewer, payload) {
  ...
  lines.push('  "score": <integer 1 to 10, where 1 = reject, 10 = accept as-is>,');
  lines.push('  "summary": "<two to three sentence reviewer summary: what the paper does, your overall read>",');
  lines.push('  "strengths": ["<specific strength>", "<another>", "<another>"],');
  lines.push('  "major_issues": [');
  lines.push('    { "title": "<short headline>", "detail": "<two to four sentences naming the issue, citing specific sections or claims>", "actionable": "<one sentence: what the author can do about it>" }');
  lines.push('  ],');
  lines.push('  "minor_issues": [');
  lines.push('    { "title": "<short headline>", "detail": "<one to two sentences>" }');
  lines.push('  ],');
  lines.push('  "questions_for_authors": ["<specific question phrased the way you would ask in the report>", "<another>"],');
  lines.push('  "confidential_comments_to_editor": "<two to four sentences for the editor only; your honest read on whether this is recoverable and what the editor should weight>"');
  lines.push('}');
  lines.push('');
  lines.push('QUANTITY GUIDELINES');
  lines.push('  - strengths: 2 to 4');
  lines.push('  - major_issues: 2 to 6 (the ones that drive your decision)');
  lines.push('  - minor_issues: 2 to 8');
  lines.push('  - questions_for_authors: 3 to 7');
  lines.push('');
  lines.push('CONTENT RULES');
  lines.push('  - Stay IN CHARACTER. Use your voice, your lens. Other reviewers on this panel are coming from different angles; do not try to cover all bases.');
  lines.push('  - Tie each issue to a SPECIFIC section, table, figure, or claim in the paper. "Page X" or "Methods, paragraph 2" or "Table 3" anchors.');
  lines.push('  - DO NOT invent results, citations, or content not in the paper. If a section is missing or unclear, say so as a finding rather than guessing.');
  lines.push('  - Your decision should reflect your lens. A Methodologist who sees fatal Approach issues should recommend major revisions or reject regardless of how interesting the topic is. A Domain Expert who sees no contribution to the field should reject even if methods are clean.');
  lines.push('  - The confidential comment to editor is where you say the thing you would not put in the author-facing review: candor about novelty, fit, replicability, anything you would only tell the editor.');
  lines.push('  - No em dashes. Use commas, periods, or restructure. Em dashes are banned.');
  lines.push('  - No marketing-cliche adjectives. "Robust", "leverage", "synergy", "novel" as filler are cut.');
  return lines.join('\n');
}
...
function buildUserContent(payload) {
  ...
  if (paper_pdf && paper_pdf.data) {
    blocks.push({
      type: 'document',
      source: { type: 'base64', media_type: 'application/pdf', data: paper_pdf.data },
    });
    blocks.push({
      type: 'text',
      text: 'The paper is attached above as a PDF. Read it carefully and produce your reviewer report. Return ONLY the JSON object specified.',
    });
  } else {
    blocks.push({
      type: 'text',
      text: 'PAPER TEXT (pasted by the author since no PDF was uploaded):\n\n' + paper_text + '\n\nProduce your reviewer report. Return ONLY the JSON object specified.',
    });
  }
  return blocks;
}
...
function shapeReviewerReport(report, reviewerKey) {
  ...
  return {
    ...
    role: r.role,
    desc: r.desc,
    portrait_open: r.portrait_open,
    portrait_closed: r.portrait_closed,
    portrait_card: r.portrait_card,
    decision: decision,
    score: Number.isFinite(scoreNum) ? Math.max(1, Math.min(10, scoreNum)) : 5,
...
function computeEditorDecision(reviewerReports) {
  // Decision precedence: reject > major-revisions > minor-revisions > accept
  // Composite: take the strictest decision a majority of reviewers gave, else the strictest single decision.
  if (!reviewerReports.length) {
    return { consensus: 'major-revisions', consensus_basis: 'No reviews completed', avg_score: 0 };
  }
  ...
  for (const d of order) {
    if ((counts[d] || 0) > half) { consensus = d; break; }
  }
  if (!consensus) {
    // No majority; take the strictest decision any reviewer gave
    for (const d of order) {
      if (counts[d]) { consensus = d; break; }
    }
  }
  ...
  return {
    consensus: consensus || 'major-revisions',
    consensus_basis: breakdown,
    avg_score,
  };
}
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    // Expose the reviewer roster so the UI can render the gallery without
    // duplicating the persona data.
    const roster = Object.keys(REVIEWERS).map((k) => ({
      id: k,
      name: REVIEWERS[k].name,
      role: REVIEWERS[k].role,
      desc: REVIEWERS[k].desc,
      portrait_open: REVIEWERS[k].portrait_open,
      portrait_closed: REVIEWERS[k].portrait_closed,
      best_for: REVIEWERS[k].best_for,
    }));
    return json(200, { ok: true, reviewers: roster });
  }
  ...
  // Paper input: PDF preferred, text fallback
  ...
  if (!paper_pdf && (!paper_text || paper_text.length < 200)) {
    return json(400, { error: 'Either paper_pdf or paper_text (min 200 chars) is required' });
  }
  if (paper_pdf && (paper_pdf.data || '').length > MAX_PDF_BYTES) {
    return json(413, { error: 'PDF too large; please downscale to under 4MB base64 or paste text' });
  }
  ...
  // Reviewer selection: explicit or auto
  let reviewerIds;
  if (Array.isArray(body.reviewers) && body.reviewers.length) {
    reviewerIds = body.reviewers
      .map((x) => String(x).toLowerCase())
      .filter((x) => VALID_REVIEWER_IDS.indexOf(x) >= 0);
    if (!reviewerIds.length) return json(400, { error: 'no valid reviewer ids in reviewers[]' });
    reviewerIds = reviewerIds.slice(0, 5); // cap fan-out
  } else {
  ...
  // Fan out: one Anthropic call per reviewer in parallel.
  const calls = reviewerIds.map(async (rid) => {
    ...
    try {
      ...
      console.error('[journal-reviewer-panel] reviewer ' + rid + ' failed:', err && err.message);
      return {
        id: rid,
        name: REVIEWERS[rid].name,
        role: REVIEWERS[rid].role,
        desc: REVIEWERS[rid].desc,
        portrait_open: REVIEWERS[rid].portrait_open,
        portrait_closed: REVIEWERS[rid].portrait_closed,
        portrait_card: REVIEWERS[rid].portrait_card,
        decision: null,
        score: null,
  ...
  if (!reviewerReports.length) return json(502, { error: 'all reviewers failed' });
