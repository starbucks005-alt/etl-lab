/* specialist-delia-ask — sync trigger for Delia Marsh's Development backpack. */
...
function newJobId() {
  ...
  return 'dm-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  const bgUrl = base + '/.netlify/functions/specialist-delia-background';
  ...
  return {
    ...
    body: JSON.stringify({ ok: true, job_id: jobId, polling_endpoint: '/.netlify/functions/specialist-delia-status?job_id=' + jobId }),
  };
};
