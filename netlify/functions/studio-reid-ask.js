/* studio-reid-ask — sync trigger for Reid Callum's marketing backpack.
   Returns a job_id immediately and kicks the background engine. */
...
function newJobId() {
  ...
  return 'rd-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  const bgUrl = base + '/.netlify/functions/studio-reid-background';
  ...
  return {
    ...
    body: JSON.stringify({ ok: true, job_id: jobId, polling_endpoint: '/.netlify/functions/studio-reid-status?job_id=' + jobId }),
  };
};
