/* studio-yuki-ask — sync trigger for Yuki Mendel's brand direction generator.
   POST { brief, owner_site?, owner_name?, owner_context? }
   Returns job_id, fires the background generator. */
...
function newJobId() {
  ...
  return 'yuk-' + stamp + '-' + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function(event) {
  ...
  try {
    fetch(base + '/.netlify/functions/studio-yuki-background', {
      method: 'POST',
  ...
  return {
    ...
    body: JSON.stringify({
      ok: true,
      job_id: jobId,
      polling_endpoint: '/.netlify/functions/studio-yuki-status?job_id=' + jobId,
    }),
  };
};
